...
data = [
    ...
    {'agentId': 4497, 'agentName': 'Ashburn, VA (Trial)', 'agentType': 'Cloud', 'countryId': 'US', 'enabled': None, 'location': 'Ashburn Area', 'network': None, 'prefix': 'not_ipv4', 'agentState': None, 'hostname': None, 'tagAll': 'X', 'tagCustomZ': 'X'},
    {'agentId': 4500, 'agentName': 'Sydney, Australia (Trial)', 'agentType': 'Cloud', 'countryId': 'AU', 'enabled': None, 'location': 'New South Wales, Australia', 'network': 'not_match_pattern', 'prefix': None, 'agentState': None, 'hostname': None, 'tagAll': 'X', 'tagCustomZ': 'X'},
...
# Before we validate we need to do a save.  This should be fixed in future
tables.save_xlsx('tempfile')

# We can validate an individual table against a supplied schema dictionary
# The results are returned as a dict
result = tables.validate_table_data_with_schema('test.schema2', test_schema)
print(result)

# Alternatively we can validate all the tables against the available schema
# Results are returned as a dict with the option to additionally print to stdout
results = tables.validate_table_data(stdout=True)
